...
export default async function EditAdminPostPage({
  ...
  const workflowCard = (
    ...
    >
      <div className="mb-5 rounded-[1.5rem] border border-white/10 bg-black/20 px-4 py-4">
        <div className="flex flex-wrap items-center gap-3">
          <span className="rounded-full bg-white/10 px-3 py-1 text-[11px] uppercase tracking-[0.2em] text-white">
            Release
          </span>
          <span className="text-sm text-stone-100">{releaseBadge}</span>
        </div>
        ...
        {post.hasPublishedVersion ? (
          <div className="mt-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-stone-300">
  ...
  const reviewCard = (
    ...
    >
      <div className="space-y-5">
        ...
        <div className="rounded-[1.5rem] border border-amber-300/20 bg-amber-400/10 px-4 py-4 text-sm leading-7 text-amber-100">
          {post.hasUnpublishedChanges
        ...
        <div className="rounded-[1.5rem] border border-white/10 bg-black/20 px-4 py-4">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-emerald-400/20 bg-emerald-500/10 px-4 py-3">
              ...
              <p className="mt-2 font-serif text-2xl text-emerald-50">
            ...
            <div className="rounded-2xl border border-red-400/20 bg-red-500/10 px-4 py-3">
              ...
              <p className="mt-2 font-serif text-2xl text-red-50">
                {markdownDiff.removedCount}
              </p>
              <p className="mt-1 text-xs leading-6 text-red-100/80">
                공개본에서 빠지는 라인
              </p>
            </div>
            <div className="rounded-2xl border border-amber-300/20 bg-amber-400/10 px-4 py-3">
              ...
              <p className="mt-2 font-serif text-2xl text-amber-50">
            ...
            <div className="mt-4 space-y-4">
              {markdownDiff.blocks.map((block, index) => (
                ...
                >
                  ...
                  {block.before.length ? (
                    <div className="mt-3 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-3">
                      <p className="mb-2 text-[11px] uppercase tracking-[0.2em] text-red-200">
                        공개본
                      </p>
                  ...
                  {block.after.length ? (
                    <div className="mt-3 rounded-xl border border-emerald-400/20 bg-emerald-500/10 px-3 py-3">
                      <p className="mb-2 text-[11px] uppercase tracking-[0.2em] text-emerald-200">
                        작업본
                      </p>
  ...
  const historyCard = (
    ...
    >
      <div className="space-y-4">
        ...
        {olderEvents.length ? (
          <details className="group rounded-[1.5rem] border border-white/10 bg-black/10 px-4 py-4">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm text-stone-300 marker:hidden">
  ...
  function getWorkflowButtonClass(options: {
    ...
    if (options.active) {
      ...
      if (options.tone === "amber") {
        return "rounded-full border border-amber-200 bg-amber-300/20 px-4 py-2 text-sm text-amber-50";
      }

      if (options.tone === "danger") {
        return "rounded-full border border-red-300/60 bg-red-500/15 px-4 py-2 text-sm text-red-100";
      }

      return "rounded-full border border-white/40 bg-white/10 px-4 py-2 text-sm text-white";
    }

    if (options.tone === "solid") {
      return "rounded-full border border-white/20 bg-white/8 px-4 py-2 text-sm font-medium text-white transition hover:border-white/40 hover:bg-white/14 disabled:cursor-not-allowed disabled:border-white/10 disabled:bg-white/6 disabled:text-white/45";
    }

    if (options.tone === "amber") {
      return "rounded-full border border-amber-300/25 px-4 py-2 text-sm text-amber-100 transition hover:border-amber-200/45 disabled:cursor-not-allowed disabled:border-amber-300/10 disabled:text-amber-100/40";
    }

    if (options.tone === "danger") {
      return "rounded-full border border-red-400/25 px-4 py-2 text-sm text-red-200 transition hover:border-red-300/40 disabled:cursor-not-allowed disabled:border-red-400/10 disabled:text-red-200/40";
    }

    return "rounded-full border border-white/15 px-4 py-2 text-sm text-stone-100 transition hover:border-white/35 disabled:cursor-not-allowed disabled:border-white/10 disabled:text-stone-100/40";
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="mt-3 font-serif text-4xl text-white">{post.title}</h2>
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <span
            className={`rounded-full px-3 py-1 text-xs uppercase tracking-[0.2em] ${
              post.hasUnpublishedChanges
                ? "border border-amber-300/25 bg-amber-300/15 text-amber-100"
                : post.hasPublishedVersion
